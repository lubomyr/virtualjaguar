...
//		mainSurface = SDL_SetVideoMode(VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT_NTSC, 16, mainSurfaceFlags);
		...
		// because of the NTSC <-> PAL resize, we only have to re-create the texture
		// instead of initializing the entire OpenGL texture en screens.
		mainSurface = SDL_SetVideoMode(SDL_ListModes(NULL, 0)[0]->w, SDL_ListModes(NULL, 0)[0]->h, 32, mainSurfaceFlags);//*/
...
//	backbuffer = (int16_t *)malloc(845 * 525 * sizeof(int16_t));
...
//	memset(backbuffer, 0x44, VIRTUAL_SCREEN_WIDTH * VIRTUAL_SCREEN_HEIGHT_NTSC * sizeof(int16_t));
	memset(backbuffer, 0x44, VIRTUAL_SCREEN_WIDTH *
		(vjs.hardwareTypeNTSC ? VIRTUAL_SCREEN_HEIGHT_NTSC : VIRTUAL_SCREEN_HEIGHT_PAL)
		* sizeof(uint32_t));
	JaguarSetScreenPitch(GetSDLScreenWidthInPixels());
        JaguarSetScreenBuffer(backbuffer);
...
//	memcpy(surface->pixels, backbuffer, TOMGetVideoModeWidth() * TOMGetVideoModeHeight() * 2);
...
//NOTE: This does *NOT* work with OpenGL rendering! !!! FIX !!!
	...
	mainSurface = SDL_SetVideoMode(tomWidth, tomHeight, 16, mainSurfaceFlags);
